...
def test__parse_dependency_string__can_parse_pypi_dependency():
  ...
  assert parse_dependency_string('kek [docs,  internal] ^1.0.0; python_version="<2.7" --hash=sha1:123456') == \
...
def test__parse_dependency_string__can_parse_git_dependency():
  assert parse_dependency_string('kek@git+https://github.com/kek/kek.git; os_name="nt"') == \
    GitDependency('kek', 'https://github.com/kek/kek.git', markers='os_name="nt"')
  assert parse_dependency_string('kek[docs] @ git+https://github.com/kek/kek.git') == \
    GitDependency('kek', 'https://github.com/kek/kek.git', extras=['docs'])
  assert parse_dependency_string('kek[docs] @ git+https://github.com/kek/kek.git#branch=develop') == \
...
def test__parse_dependency_string__can_parse_url_dependency():
  assert parse_dependency_string('kek @ https://blog.org/kek-1.2.1.tar.gz#sha1=123456&sha1=654321') == \
    UrlDependency('kek', 'https://blog.org/kek-1.2.1.tar.gz', hashes=['sha1:123456', 'sha1:654321'])
  assert parse_dependency_string('kek[docs] @ https://blog.org/kek-1.2.1.tar.gz#sha1=123456 --hash=sha1:654321') == \
...
def test__parse_dependency_config__various_configurations():
  ...
  assert parse_dependency_config('foo', 'git+https://github.com/foo/foo.git') == GitDependency('foo', 'https://github.com/foo/foo.git')
