...
from slam.application import Application, Command, argument
from slam.plugins import ApplicationPlugin


class RunCommandPlugin(Command, ApplicationPlugin):
  """ Run a command configured in <code>[tool.slam.run]</code>"""

  name = "run"
  arguments = [
    argument(
      "cmd",
      description="The name of the command to run as it is defined under the <code>slam.run</code> section.",
    ),
  ]

  def load_configuration(self, app: Application) -> dict[str, str]:
    return app.main_project().raw_config().get('run', {})
